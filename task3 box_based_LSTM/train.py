...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (texts, labels) in enumerate(train_loader):
        ...
        # We use batch 1 since we dont want boxes number padding
        texts = torch.cat(texts, dim=0)  # [Tensor(n, 50)] -> tensor  (n, 50)
        labels = torch.cat(labels, dim=0)  # [Tensor(n)] -> tensor (n  x 1)
...
def validate(val_loader, model, criterion, train_loader):
    """
    One epoch's validation.
        ...
        for i, (texts, labels) in enumerate(train_loader):
            ...
            if i % print_freq == 0:
                print('[{0}/{1}]\t'
                      'Batch Time {batch_time.val:.3f} ({batch_time.avg:.3f})\t'
                      'Loss {loss.val:.4f} ({loss.avg:.4f})\t' '\nAccuracy {accuracy:.3f}%\t' 'F1 {F1:.3f}\t'
                      'Precision {Precision:.3f}\t' 'Recall {Recall:.3f}'.format(i, len(val_loader),
                                                                                 batch_time=batch_time,
                                                                                 loss=losses, accuracy=acc * 100,
